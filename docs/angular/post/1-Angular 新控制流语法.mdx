---
title: Angular 新控制流语法 (@if, @for, @switch)
---
...
```html
<div *ngIf="condition">This content is displayed if the condition is true.</div>
```
...
```html
@if (condition) {
<div>This content is displayed if the condition is true.</div>
} @else {
<div>This content is displayed if the condition is false.</div>
}
```
