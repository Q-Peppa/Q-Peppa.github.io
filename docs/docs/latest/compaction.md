...
在多轮 agent 运行中，Pi 会在工具执行完毕、结果追加到上下文后、开始下一条助手响应之前检查该阈值。如果超过阈值，Pi 会在同一次 agent 运行内进行压缩，并以摘要和保留的消息恢复运行。当已完成的工具批次终止了运行且没有排队消息需要另一次响应时，它会跳过这种轮次之间的检查。Pi 还会在新用户 Prompt 之前以及低层 agent 运行结束时检查阈值。
