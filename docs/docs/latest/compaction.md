...
两种机制使用相同的结构化摘要格式，并累积跟踪文件操作。压缩和分支摘要请求使用新的路由会话 ID，并在 Provider 支持的情况下禁用 prompt 缓存写入，因为这些一次性 prompt 不太可能被重用。
...
在多轮 agent 运行中，Pi 会在工具执行完毕、结果追加之后、开始下一条助手响应之前检查规范化的投影上下文。如果超过阈值，Pi 会在 `prepareNextTurn` 期间进行压缩，然后在 `turn_start` 之前执行既有的追赶 steering 轮询。当已完成的工具批次终止了运行且没有排队消息需要另一次响应时，它会跳过这种轮次之间的检查。Pi 还会在新用户 Prompt 之前检查，并在低层运行结束后执行最后一次尝试的溢出恢复。

Provider 的上下文溢出错误，或提前出现的最终 `stopReason: "length"`，可以选中一次「压缩并重试」的恢复尝试。带工具调用的 length 响应会保留其合成的失败工具结果，并遵循常规的工具/队列调度，而不是强制结束运行。

你也可以使用 `/compact [instructions]` 手动触发，其中可选的 instructions 用于聚焦摘要内容。

### 工作原理

1. **找到切割点**：在最终确定的会话投影中向后遍历，累积 Token 估算，直到达到 `keepRecentTokens`（默认 20k，可在 `~/.pi/agent/settings.json` 或 `<project-dir>/.pi/settings.json` 中配置）
2. **提取消息**：收集投影后的消息，从上次保留边界（或会话开始）到切割点
...
在重复压缩时，摘要跨度从上次压缩的保留边界（`firstKeptEntryId`）开始，而不是从压缩条目本身开始；如果无法在路径中找到该保留条目，则回退到上次压缩之后的那一个条目。不保留任何内容的压缩会把自身的 ID 记为 `firstKeptEntryId`；重复压缩从该条目之后开始。这样，那些在早期压缩中幸存下来的消息也会被纳入后续的摘要处理中。Pi 还会在写入新的 `CompactionEntry` 之前，从重建且经过上下文编辑的会话投影中重新计算 `tokensBefore`，因此 Token 计数反映了被替换的实际压缩前上下文。被省略的原始条目仍然存储，但不影响切割点选择、摘要、检查点或 Token 估算。

### 溢出与长度恢复的顺序

恢复会保持既有的生命周期与队列顺序。已完成的尝试对 `turn_end` 和 `agent_end` 仍然可见；运行后的恢复随后修复持久化的模型上下文，然后才以一次全新运行开始重试：
...
→ 溢出/长度：运行 session_before_compact，成功时追加压缩
→ 以一次全新运行开始重试
```

如果恢复压缩失败或被取消，Pi 会保留省略编辑，不追加压缩，也不调度内部重试。已有的排队工作仍由常规 steering 和 follow-up 规则管理。`agent_before_settle` 看到的是恢复处理之后修复过的投影。原始转录历史、导出、计费总计和历史搜索扩展仍然可以检查被省略的尝试。
...
准备阶段只在该后缀包含被省略的 assistant 尝试、且没有未省略的上下文生成条目时，才会把保留边界推进到上下文不可见的后缀。恢复用的 `context_edit` 省略满足此规则；本身上下文不可见的元数据可以与它们共存。单独的元数据和新追加的自定义消息不会移动切割点。影响候选输入或摘要前缀的替换编辑同样会阻止推进，因为被省略的 assistant 回答的是编辑前的输入；最终被省略的后缀条目的替换仍然安全。这样，超出预算的恢复输入可以被摘要，同时保留让被放弃的尝试保持省略的编辑，而簿记本身不会改变新模型输入是否被原样保留。
