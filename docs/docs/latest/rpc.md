...
参见 [`test/rpc-example.ts`](https://github.com/earendil-works/pi/tree/main/packages/coding-agent/test/rpc-example.ts) 获取完整的交互式示例，或 [`src/modes/rpc/rpc-client.ts`](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/src/modes/rpc/rpc-client.ts) 获取带类型定义的客户端实现。

关于处理扩展 UI 协议的完整示例，参见 [`examples/rpc-extension-ui.ts`](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/examples/rpc-extension-ui.ts)，它与 [`examples/extensions/rpc-demo.ts`](https://github.com/earendil-works/pi/tree/main/packages/coding-agent/examples/extensions/rpc-demo.ts) 扩展配合使用。
