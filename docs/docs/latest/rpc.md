...
`"xhigh"` 和 `"max"` 仅在所选模型支持时暴露。部分模型（包括 GPT-5.6）同时暴露两者。
...
`estimatedTokensAfter` 是对压缩后立即重建的消息上下文的启发式估算，而非 Provider 精确的 Token 计数。`usage` 报告生成摘要的 LLM 调用，自定义压缩处理程序可能会省略此字段。
...
`tokens` 和 `cost` 包括助手消息、工具报告的用量以及整个会话中的压缩/分支摘要生成。`contextUsage` 包含用于压缩和底部显示的实际当前上下文窗口估计值。
...
| `agent_end`                         | 一次底层 agent 运行完成（可能后跟重试、压缩或排队的继续消息）        |
| `agent_settled`                     | Agent 运行完全 settled；没有剩余的自动重试、压缩重试或排队的继续消息 |
| `turn_start`                        | 新回合开始                                                           |
...
一次底层 agent 运行完成时发出。包含此次运行中生成的所有消息。如果 `willRetry` 为 true，将进行自动重试。
...
会话级运行完全 settled 后发出。此时 Pi 不会再通过重试、压缩重试或排队的 follow-up 消息自动继续。
...
在压缩或分支摘要生成因临时 Provider 错误而重试时发出。这些事件使用与自动助手轮次重试相同的重试设置。
...
`usage` 是可选的，报告工具执行的嵌套 LLM 工作。当存在时，它会计入会话 Token 和成本总计。
