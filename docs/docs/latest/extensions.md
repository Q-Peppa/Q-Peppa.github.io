...
转换在处理程序间链式拼接。参见 [input-transform.ts](https://github.com/earendil-works/pi/tree/main/packages/coding-agent/examples/extensions/input-transform.ts)。
...
pi.registerProvider("my-proxy", {
  ...
  apiKey: "PROXY_API_KEY",  // 环境变量名或字面值
...
- `apiKey` - API 密钥或环境变量名称。定义模型时需要（除非提供了 `oauth`）。
...
ctx.ui.setFooter((tui, theme) => ({
  render(width) { return [theme.fg("dim", "自定义底部栏")]; },
  invalidate() {},
}));
