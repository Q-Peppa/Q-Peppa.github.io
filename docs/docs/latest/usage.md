...
| `-h`、`--help`                  | 显示帮助                                        |
| `--alt`                         | 在交互模式中使用备用屏幕 TUI，支持应用自控滚动  |
| `-v`、`--version`               | 显示版本                                        |

当 `--alt` 激活时，内联图片在支持 Kitty 图形协议的终端（包括 Kitty 和 Ghostty）中正常工作。在 iTerm2 中它们渲染为文本占位符，因为其内联图片协议无法在应用自控滚动期间删除或裁剪渲染。不使用 `--alt` 时，iTerm2 内联图片继续正常渲染。
