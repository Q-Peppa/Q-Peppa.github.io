...
未带 `git:` 前缀时，仅接受协议 URL（`https://`、`http://`、`ssh://`、`git://`）。带有 `git:` 前缀时，接受简写格式，包括 `github.com/user/repo` 和 `git@github.com:user/repo`。
...
- `skills/` —— 递归查找 `SKILL.md` 文件夹，加载顶级 `.md` 文件作为 Skills
- `prompts/` —— 加载 `.md` 文件
- `themes/` —— 加载 `.json` 文件
